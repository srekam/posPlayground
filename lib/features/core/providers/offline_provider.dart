...
class OfflineNotifier extends StateNotifier<OfflineState> {
  ...
  void addOutboxEvent(OutboxEventType type, Map<String, dynamic> payload) {
    ...
    final pendingCount =
        updatedEvents.where((e) => e.status == OutboxEventStatus.queued).length;
  ...
  void _updateEventStatus(String eventId, OutboxEventStatus status,
      {String? error}) {
    final updatedEvents = state.outboxEvents.map((event) {
      if (event.id == eventId) {
        return event.copyWith(
          status: status,
          error: error,
          retryCount: status == OutboxEventStatus.failed
              ? event.retryCount + 1
              : event.retryCount,
        );
      }
